...
if not SKIP_CUDA_BUILD and not IS_ROCM:
    ...
    feature_flags = []
    if os.getenv("FLASH_ATTENTION_DISABLE_DROPOUT", "FALSE").upper() in ("TRUE", "1", "ON", "YES"):
        feature_flags.append("-DFLASHATTENTION_DISABLE_DROPOUT")
    if os.getenv("FLASH_ATTENTION_DISABLE_ALIBI", "FALSE").upper() in ("TRUE", "1", "ON", "YES"):
        feature_flags.append("-DFLASHATTENTION_DISABLE_ALIBI")
    if os.getenv("FLASH_ATTENTION_DISABLE_SOFTCAP", "FALSE").upper() in ("TRUE", "1", "ON", "YES"):
        feature_flags.append("-DFLASHATTENTION_DISABLE_SOFTCAP")
    if os.getenv("FLASH_ATTENTION_DISABLE_UNEVEN_K", "FALSE").upper() in ("TRUE", "1", "ON", "YES"):
        feature_flags.append("-DFLASHATTENTION_DISABLE_UNEVEN_K")
    if os.getenv("FLASH_ATTENTION_DISABLE_LOCAL", "FALSE").upper() in ("TRUE", "1", "ON", "YES"):
        feature_flags.append("-DFLASHATTENTION_DISABLE_LOCAL")

    # Filter head dimensions if requested via FLASH_ATTN_HEAD_DIMS (e.g. "64,128" or "all")
    all_hdims = [32, 64, 96, 128, 192, 256]
    raw_head_dims = os.getenv("FLASH_ATTN_HEAD_DIMS", "").strip()
    if raw_head_dims and raw_head_dims.lower() not in ("all", "*", "none"):
        requested_hdims = [int(h.strip()) for h in re.split(r"[,;]+", raw_head_dims) if h.strip()]
        for h in requested_hdims:
            if h not in all_hdims:
                raise ValueError(f"Invalid head dimension {h}. Supported dimensions: {all_hdims}")
        enabled_hdims = requested_hdims
    else:
        enabled_hdims = all_hdims

    for h in all_hdims:
        if h not in enabled_hdims:
            feature_flags.append(f"-DFLASHATTENTION_DISABLE_HDIM{h}")
    ...
    fa2_sources = []
    for s in all_fa2_sources:
        m = re.search(r"_hdim(\d+)_", s)
        if m:
            if int(m.group(1)) in enabled_hdims:
                fa2_sources.append(s)
        else:
            fa2_sources.append(s)
